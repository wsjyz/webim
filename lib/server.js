        ...
        if (false) {
            ...
            socket.on('join_room', function (data) {
                ...
                for(var index in cookieArr){
                    var aStr = cookieArr[index];
                    if(aStr.indexOf("corp_code=") != -1){
                        corpCode = aStr.substring(aStr.indexOf("corp_code=")+10,aStr.length)
                    }
                }
...
//                msg.bothUserId=socket.user.userId+"|"+data.toUserId;
            ...
            socket.on("select_current_video", function (){
                var curMedias = roomMedias[socket.room];
                if(curMedias.length + 1 > settings.limit_video_counts){
                    socket.emit("select_current_video",{msg:"当前音视频数量已达到上限，请等待他人退出",canJoin:false});
                }else{
                    socket.emit("select_current_video",{canJoin:true});
                }
            });
            socket.on("broadcast_join_rtc_room", function (data) {
                var curMedias = roomMedias[socket.room];
                var testUserName = '';
                for(var x in curMedias){
                    testUserName += curMedias[x].userName + ',';
                }
                console.log(testUserName);
                //console.log(curMedias.length + "--"+settings.limit_video_counts);
                if(curMedias.length + 1 <= settings.limit_video_counts) {
                    socket.media = true;
                    var msg;
                    var curMedias = roomMedias[socket.room];
                    curMedias.push(socket.user);
                    console.log("join count " + curMedias.length);
                    if (curMedias.length == 1) {
            ...
            socket.on("broadcast_quit_rtc_room",function(){
                ...
                console.log("quit count "+curMedias.length);
            ...
            socket.on("disconnect", function () {
                ...
                console.log("dis count "+curMedias.length);
            });
